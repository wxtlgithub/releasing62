...
Js.Center.Alert.AlertInfo.Info = function(node){
	if(Ext.get("Js.Center.Alert.AlertInfo.MainPanel") == null){
		//加载通道组列表，避免报警短信设置页面首次打开显示通道组编号问题
		Js.Center.Common.ProductStore.reload();
		//=================定义网关监控Grid==========Start====================
		// ============ 指定列参数==============
        var gatewayfields = ["numtaskid","numgwid","vc2gatewayname","module_status","connection_status","response_count","submit_speed","numstate","QUEUE_COUNT"];
		//=============================定义通用模块数据源
...
//          needPage:false,
      ...
      //=================定义Response失败率监控Grid==========Start====================
		// ============ 指定列参数==============
        var respfailFields = ["numtaskid","numgwid","vc2gatewayname","response_count","response_error","response_error_pct","numstate"];
		//=============================定义通用模块数据源
...
//          needPage:false,
      ...
      //=================定义Report失败率监控Grid==========Start====================
		// ============ 指定列参数==============
        var reportfailFields = ["numtaskid","numgwid","vc2gatewayname","rpt_count","rpt_fail_count","rpt_error_pct","numstate"];
		//=============================定义通用模块数据源
...
//          needPage:false,
      ...
      //=================定义延时监控Grid==========Start====================
		// ============ 指定列参数==============
        var delayFields = ["numtaskid","numgwid","vc2gatewayname","sms_delay","sms_delay_average","numstate"];
		//=============================定义通用模块数据源
	...
	Js.Center.Alert.AlertInfo.getNewMonitorInfoFromDB = function(nodeId){
		...
		Js.Center.Alert.AlertInfo.gatewayInfostore.load({
            ...
            "callback": function(r,options,success){
            	if(success){
            		Js.Center.Alert.AlertInfo.getNewMonitorInfo('1','start','232','1');
            	}
		...
		Js.Center.Alert.AlertInfo.delayInfostore.load({
        	...
        	"callback": function(r,options,success){
        		if(success){
        			Js.Center.Alert.AlertInfo.getNewMonitorInfo('1','start','232','2');
        		}
...
Js.Center.Alert.AlertInfo.getNewMonitorInfo = function(id,command,param,typeId){
	try{
		
		var urlParam="?cmd=-U"+NUMCOMUNICATIONNODEID+" -Cviewall"; //"?flag='"+ flag +"'flag&command='"+ command +"'&id=" + id
		var commonMonitorData = doSynRequest(Js.Center.Alert.AlertMonitorQueryURL + urlParam);
		//var commonMonitorData = doSynRequest("testcmd.jsp" + urlParam);
		var gatewayDataStore =Js.Center.Alert.AlertInfo.gatewayInfostore;
//		var respfailDataStore = Js.Center.Alert.AlertInfo.respFailInfostore;
//		var reportfailDataStore = Js.Center.Alert.AlertInfo.reportfailInfostore;
...
//					//更新Reponse失败率监控信息
//					for (var j = 0; j < respfailDataStore.data.length; j++) {
//						if (commonMonitorData.data[i].modelid == respfailDataStore.data.items[j].data.numtaskid) {
//							if(commonMonitorData.data[i].key == "RESPONSE_COUNT"){
//								respfailDataStore.getById(respfailDataStore.data.items[j].data.numgwid).set('response_count', commonMonitorData.data[i].value);
//							}
//							if(commonMonitorData.data[i].key == "RESPONSE_ERROR"){
//								respfailDataStore.getById(respfailDataStore.data.items[j].data.numgwid).set('response_error', commonMonitorData.data[i].value);
//							}
//							if(commonMonitorData.data[i].key == "RESPONSE_ERROR_PCT"){
//								respfailDataStore.getById(respfailDataStore.data.items[j].data.numgwid).set('response_error_pct', commonMonitorData.data[i].value);
//							}
//							respfailDataStore.getById(respfailDataStore.data.items[j].data.numgwid).commit();
//						}
//					}
//					//更新report失败率监控信息
//					for (var j = 0; j < reportfailDataStore.data.length; j++) {
//						if (commonMonitorData.data[i].modelid == reportfailDataStore.data.items[j].data.numtaskid) {
//							if(commonMonitorData.data[i].key == "RPT_COUNT"){
//								reportfailDataStore.getById(reportfailDataStore.data.items[j].data.numgwid).set('rpt_count', commonMonitorData.data[i].value);
//							}
//							if(commonMonitorData.data[i].key == "RPT_FAIL_COUNT"){
//								reportfailDataStore.getById(reportfailDataStore.data.items[j].data.numgwid).set('rpt_fail_count', commonMonitorData.data[i].value);
//							}
//							if(commonMonitorData.data[i].key == "RPT_ERROR_PCT"){
//								reportfailDataStore.getById(reportfailDataStore.data.items[j].data.numgwid).set('rpt_error_pct', commonMonitorData.data[i].value);
//							}
//							reportfailDataStore.getById(reportfailDataStore.data.items[j].data.numgwid).commit();
//						}
//					}
					//更新延时监控信息
					if(typeId == "2"){
						for (var j = 0; j < delayDataStore.data.length; j++) {
							if (commonMonitorData.data[i].modelid == delayDataStore.data.items[j].data.numtaskid) {
								...
								//delayDataStore.getById(delayDataStore.data.items[j].data.numgwid).commit();
							}
						}
					}
