                ...
                fromTo: function(){
            		...
            		if (selectionsArray.length > 0) {
            			...
            			if(selectItems.length > 0){
                			for(var i = 0; i < selectItems.length; i++){
                				if(i == selectItems.length - 1){
                    				toItemIds += selectItems[i].json.numsvcid;
                				} else {
                    				toItemIds += selectItems[i].json.numsvcid + ",";
                				}
                			}
            			...
            			for (var i=0; i<selectionsArray.length; i++) {
                			...
                			if(toItemIds != "") {
                				returnInfo = ajaxSyncCall(Js.Center.Business.ProductURL,"flag=checkselectnumsvcid&numsvcid=" + record.get("numsvcid") + "&selectsvcid=" + toItemIds);
                    			//已选栏已经有通道则进行网关和地区验证
            					if(true == returnInfo.success){
                    				records.push(record);
                        			if("" == toItemIds){
                        				toItemIds = record.get("numsvcid");
                        			} else {
                        				toItemIds = record.get("numsvcid") + "," + toItemIds;
                        			}
        						} else {
        							//包含相同的网关和地区
        							Ext.Msg.alert("提示信息", "'" + record.get("vc2svcname") +"' 与 '" + returnInfo.info + "' 网关运营商及地区相同请确认");
        							return "";
        						} 
                			} else {
                				records.push(record);
            ...
            listeners: {
                "show": function(){
                    ...
                    producynoPermitsvcStore.baseParams = {
                        flag: 'selectnopermitbypid',
                        servicetype:Js.Center.Business.productpolicy.window.updateRecord.get("vc2servicetype"),
                    ...
                    producynoPermitsvcStore.reload({
                        ...
                        callback: function(){
                        	//隐藏无效产品
                        	if(this.getById('0') != null){
                        		this.remove(this.getById('0'));
                        	}
                        }
                    });
                  //已授权通道数据源
