    ...
    if (Ext.get("Js.Center.SendSMS.FirstCheck.MainPanel") == null) {
        ...
        Js.Center.SendSMS.FirstCheck.infostore = new WXTL.Widgets.CommonData.GroupingStore({
            ...
            reader: new Ext.data.JsonReader({
                fields: ["numcontentid", "vc2content", "datsend", "vc2status", "numstate","numstatus","vc2numstatus", "nummessageformat", "numsendmethod", "numpriority", "vc2detail", "numcheck", "vc2reason", "datcreate", "numcreaterid", "datpresend", "numcheck1id", "datcheck1", "numcheck2id", "datcheck2", "numprenum", "numprodid", "numsource", "numsendtype", "vc2typelist", "numstatename", "numsendtypename", "nummessageformatname","numtotal","numsuccess","numfailed"],
                root: "data",
                id: "numlogid",
...
//                    var row = Js.Center.SendSMS.FirstCheck.infostore.getAt(0);
//                    //alert(row.get("numcontentid"));
//                    var sucRate;
//                    if(row.get("numtotal") != "" && row.get("numtotal") != 0){
//                        sucRate = Math.round(row.get("numsuccess")/row.get("numtotal")*1000)/10;
//                    }
//                    else{
//                        sucRate = 0;
//                    }
...
//                    Ext.get("Js.Center.SendSMS.FirstCheck.MobileListInfo").dom.innerHTML = "号码分布情况： 总数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"flag=selectexport&successtype=1\")'><font color='green'>"+ row.get("numtotal") + "</font></a> 合法数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"flag=selectexport&successtype=1\")'><font color='green'>"+ row.get("numsuccess") + "</font></a> 非法数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"flag=selectexport&successtype=1\")'><font color='green'>"+ row.get("numfailed") + "</font></a> 合法率：<font color='red'>"+sucRate+"%</font>";
//                }
            }
        });
        //==============================================================列选择模式
        ...
        //<a href='#' onclick='firstCheckDoView(\"" + record.data.numcontentid + "\",\"" + record.data.vc2content + "\")'>查看</a>
        //<a href='#' onclick='firstCheckDoPass(\"" + record.data.numcontentid + "\")'>通过</a>　<a href='#' onclick='firstCheckDoReject(\"" + record.data.numcontentid + "\")'>驳回</a>
        //==============================================================定义grid
        ...
        var selectPanel = new WXTL.Widgets.CommonPanel.QueryFormPanel({
            ...
            items: [{
                ...
                items: [{
                    ...
                    items: [{
                        //xtype: 'ClickLabel',
                        //id: "Js.Center.SendSMS.FirstCheck.MobileListInfo",
                        //fieldLabel: '号码分布情况',
            ...
            buttons: [new Ext.Button({
                ...
                handler: function(){
                    if (selectPanel.getForm().isValid()) {
                        ...
                        if(Ext.get("Js.Center.SendSMS.FirstCheck.numstatus").dom.value == 1){
                           ...
                           doAjaxWithCallBack(Js.Center.SendSMS.SmsContentUpdateURL, params,Js.Center.SendSMS.FirstCheck.callBackFunc);
...
Js.Center.SendSMS.FirstCheck.callBackFunc = function(){
	Js.Center.SendSMS.FirstCheck.infostore.reload({
        params: {
            start: 0,
            limit: 12
        ...
        callback: function(records, options, success){
        	Js.Center.SendSMS.FirstCheck.MainPanel.items.items[0].getForm().reset();
            firstCheckDoView(0);
        }
    });
};
...
function firstCheckDoView(rowIndex){
    ...
    if ( Js.Center.SendSMS.FirstCheck.infostore.getCount() > 0) {
        ...
        if(row.get("numsendtype") !=2){
            ...
            //Ext.get("Js.Center.SendSMS.FirstCheck.MobileListInfo").dom.innerHTML = "号码分布情况： 总数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"id=" + row.get("numcontentid") + "&flag=selectexport&successtype=-1\")'><font color='green'>"+ row.get("numtotal") + "</font></a> 合法数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"id=" + row.get("numcontentid") + "&flag=selectexport&successtype=1\")'><font color='green'>"+ row.get("numsuccess") + "</font></a> 非法数：<a href='#' onclick='exportData(\"" + Js.Center.SendSMS.SmsContentURL + "\",\"id=" + row.get("numcontentid") + "&flag=selectexport&successtype=0\")'><font color='green'>"+ row.get("numfailed") + "</font></a> 合法率：<font color='red'>"+sucRate+"%</font>";
        } else {
        	//客户组
            if(row.get("numtotal") > 0){
...
//        else{
//            Ext.get("Js.Center.SendSMS.FirstCheck.MobileListInfo").dom.innerHTML = "号码分布情况： 总数：<font color='green'>"+ row.get("numtotal") + "</font> 合法数：<font color='green'>"+ row.get("numsuccess") + "</font> 非法数：<font color='green'>"+ row.get("numfailed") + "</font> 合法率：<font color='red'>"+sucRate+"%</font>";
//        }
        Ext.get("Js.Center.SendSMS.FirstCheck.MobileListInfo").dom.innerHTML = "号码分布情况：" + mobileTotalInfo+"，" + mobileSucInfo+"，" + mobileFailInfo+"，" + "合法率<font color='red'>"+sucRate+"%</font>";
    }
