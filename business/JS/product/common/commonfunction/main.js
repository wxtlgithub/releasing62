...
function loadWelcome(){
    ...
    if (Js.Center.Common.userData != null) {
        ...
        */
        ...
        javaStr +='<dl><dt><span>公告信息</span></dt>' ;
        if(noticeData != null && noticeData.length > 0){
        	for(var i = 0; i < noticeData.length ; i++){
        		javaStr +=  '   <dd class="matter">' ;
        		if(noticeData[i].data.vc2filepath!=null){
//        			javaStr += "<a href='#' onClick='exportData(\"URL/business/noitce/noticequery.ashx\",\"filepath=" + noticeData[i].data.vc2filepath + "&flag=download\")' >" + noticeData[i].data.vc2bulletin + "</a></dd>";
        			javaStr += "<a href='#' onClick='noTiceDowmload("+i+")' >" + noticeData[i].data.vc2bultitle + "</a></dd>";
        		}else{
        			javaStr += noticeData[i].data.vc2bulletin +'</dd>' ;
        		}
        			
        	}
        	javaStr +='   <dd><a href="#" onClick="menuClick(\'s49\',\' 公告信息\',\'js.loadjs.loadbusinessnoticemoreinfo\')">更多</a></dd>' ;
...
function noTiceDowmload(id){
	...
	if(row.vc2filepath!=null&&row.vc2filepath!=""){
		Ext.get("Js.Center.Main.Notice.noticemsg.infoDetail.downloadinfo").dom.innerHTML = "<a href='#' onClick='windowOpen(\""+ row.vc2filepath + "\",500,500)' > <center>附件下载</center> </a>";
	}
};
