...
//    }
    ...
    var PermissionTree = new Ext.form.FormPanel({
        ...
        items: [{
            anchor: ',100%',
            xtype: "itemselector",
            id : "Js.Center.RolePermitToUser.itemselector" ,
            name: "itemselector",
            fieldLabel: "配置账户",
        	...
        	width: 200,
        	id : "Js.Center.Role.PermitToUser.Textsearch" ,
            emptyText:'Find a Class',
            listeners:{
            	render : function(f) {
            		f.el.on('keyup', function(e){Ext.getCmp("Js.Center.RolePermitToUser.itemselector").fromMultiselect.view.store.filter("vc2username",f.getValue(),true,false);
            		}, f, {buffer: 350});
				}
            }
        })],
    ...
    var southPanel = new Ext.form.FormPanel({//new Ext.Panel({
        ...
        listeners: {
            "expand": function(a, b, c){
                ...
                var productNames = "";
                //显示已授权的通道组
                var jsonProductList = eval(doSynRequest(Js.Center.Business.RoleProductURL + "?flag=selectpermitbyroleids&columnlist=numprodid,vc2name&roleids=" + Js.Center.Popedom.Role.RolePermitToUser.window.updateRecord.get("numroleid")));
        ...
        buttons: [{
            ...
            handler: function(){
                if (mainForm.isValid()) {
                    ...
                    //修改权限
                    Ext.getCmp('Js.Center.Role.PermitToUser.Textsearch').disable();
                      var  params = {
...
//    var roleDesc = Js.Center.Popedom.Role.RolePermitToUser.window.updateRecord.get("vc2roledesc");
		Ext.getCmp('Js.Center.Role.PermitToUser.Textsearch').enable();
		Ext.getCmp('Js.Center.Role.PermitToUser.Textsearch').setValue(null);
		fromStore.load({
                    params: {
                       roleid: Js.Center.Popedom.Role.RolePermitToUser.window.updateRecord.get("numroleid"),
