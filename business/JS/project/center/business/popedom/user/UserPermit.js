...
Js.Center.Popedom.UserPermit.func = function(numuserid, vc2uaccount, vc2username, numdepartid, vc2departname){
    var userData = Js.Center.Common.userData.data[0];
    if(Js.Center.Popedom.UserPermit.window==null){
	    ...
	    //=====================================================================未授权EC角色
	    var ecFromStore = new Ext.data.Store({
	        proxy: new Ext.data.HttpProxy({//数据代理 
	            url: Js.Center.Popedom.UserFuncRoleURL,
	            method: "POST"
	        }),
	    ...
	    //===========================================================================================已授权EC角色
	    var ecToStore = new Ext.data.Store({
	        proxy: new Ext.data.HttpProxy({//数据代理 
	            url: Js.Center.Popedom.UserFuncRoleURL,
	            method: "POST"
	        }),
	    ...
	    var PermissionTree = new Ext.form.FormPanel({
	        ...
	        items: [{
	            ...
	            fromTBar:[new Ext.form.TextField({    
	            	width: 200,
	            	id:"Js.Center.User.UserPermit.Textsearch",
	                emptyText:'Find a Class',
	                listeners:{
	                	render : function(f) {
	                		f.el.on('keyup', function(e){
	                			Ext.getCmp("roleitemselector").fromMultiselect.view.store.filter("vc2rolename",f.getValue(),true,false);
	                		}, f, {buffer: 350});
						}
	                }
	            })]
	        }, userData.numdepartid == 1 ? {
	            anchor: ',100%',
	            xtype: "itemselector",
	            name: "itemselector",
	            id:"ecroleitemselector",
	            fieldLabel: "EC角色",
	            dataFields: ["numroleid", "vc2rolename"],
	            toData: [""],
	            msWidth: 250,
	            ...
	            fromTBar:[new Ext.form.TextField({    
	            	width: 200,
	            	id:"Js.Center.User.UserPermit.TextsearchECRole",
	                emptyText:'Find a Class',
	                listeners:{
	                	render : function(f) {
	                		f.el.on('keyup', function(e){
	                			Ext.getCmp("ecroleitemselector").fromMultiselect.view.store.filter("vc2rolename", f.getValue(), true, false);
	                		}, f, {buffer: 350});
						}
	                }
	            })]
	    ...
	    var permissionRolePanel = new Ext.Panel({
	        //autoScroll: true,// 自动显示滚动条
	        frame: true,
	        height: userData.numdepartid == 1 ? 480 : 240,
	    ...
	    var southPanel = new Ext.form.FormPanel({//new Ext.Panel({
	        ...
	        listeners: {
	            "expand": function(a, b, c){
	                var userUgoupNames = "";
	                var productNames = "";
	                Ext.getCmp("Js.Center.User.UserPermit.Textsearch").disable();
	                if(userData.numdepartid == 1){
	               		Ext.getCmp("Js.Center.User.UserPermit.TextsearchECRole").disable();
	                }
	                var roleids = WXTL.Common.urlDecode(PermissionTree.getForm().getValues(true)).replace('itemselector=', '');
	                treePanel.root.reload();
	                if (roleids != "") {
	                    //===============================================显示已授权的通道组
	                    var jsonProductList = eval(doSynRequest(Js.Center.Business.RoleProductURL + "?flag=selectpermitbyroleids&columnlist=numprodid,vc2name&roleids=" + roleids));
	                ...
	                Ext.getCmp("Js.Center.User.UserPermit.Textsearch").enable();
	                if(userData.numdepartid == 1){
	               		Ext.getCmp("Js.Center.User.UserPermit.TextsearchECRole").enable();
	                }
	            }
	        }
	    });
	   ...
	   this.window = new WXTL.Widgets.CommonWindows.Window({
	        ...
	        loadDataStoreFunc: function(){
	            ...
	            Ext.getCmp("Js.Center.User.UserPermit.Textsearch").enable();
	            Ext.getCmp("Js.Center.User.UserPermit.Textsearch").setValue(null);
                if(userData.numdepartid == 1){
		            Ext.getCmp("Js.Center.User.UserPermit.TextsearchECRole").enable();
		            Ext.getCmp("Js.Center.User.UserPermit.TextsearchECRole").setValue(null);
                }
	            Js.Center.Popedom.User.UserStore.load({
	                    params: {
	                        departid: '',
	        ...
	        buttons: [{
	            ...
	            handler: function(){
	                if (mainForm.isValid()) {
	                    ...
	                    var idlist = PermissionTree.getForm().items.items[0].getValue();
	                    var ecRoleList = PermissionTree.getForm().items.items[1].getValue();
	                    if(idlist){
	                    	if(ecRoleList){
	                    		idlist += "," + ecRoleList;
	                    	}
