...
Js.Center.Popedom.Department.DepartmentPermit.func = function(departId, departName){
    ...
    var PermissionTree = new Ext.form.FormPanel({
        ...
        items: [{
            anchor: ',100%',
            xtype: "itemselector",
            id : "Js.Center.Depart.DepartPermit.itemselector",
            name: "itemselector",
            fieldLabel: "配置角色",
            ...
            fromTBar:[  new Ext.form.TextField({    
            	width: 200,
            	id:"Js.Center.Depart.DepartPermit.Textsearch",
                emptyText:'Find a Class',
                listeners:{
                	render : function(f) {
                		f.el.on('keyup', function(e){
                			Ext.getCmp("Js.Center.Depart.DepartPermit.itemselector").fromMultiselect.view.store.filter("vc2rolename",f.getValue(),true,false);
                		}, f, {buffer: 350});
    				}
                }
            })],
    ...
    this.window = new WXTL.Widgets.CommonWindows.Window({
        ...
        loadDataStoreFunc: function(){
			...
			Ext.getCmp('Js.Center.Depart.DepartPermit.Textsearch').enable();
			Ext.getCmp('Js.Center.Depart.DepartPermit.Textsearch').setValue(null);
			//加载授权角色数据
                fromStore.load({
                    params: {
        ...
        buttons: [{
            text: '保存',
            handler: function(){
            	 Ext.getCmp('Js.Center.Depart.DepartPermit.Textsearch').disable();
                if (mainForm.isValid()) {
                    // 弹出效果
                    Ext.MessageBox.show({
                        msg: '正在保存，请稍等...',
