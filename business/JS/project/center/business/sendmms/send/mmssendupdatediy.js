    ...
    //if (Ext.get("Js.Center.SendMMS.MMSsendUpdateDiy.MMSSendUpdatePanel") == null) {
		...
		var SendMMSUpdateDiyListFieldSet = new Ext.FormPanel({
			...
			items: [{
				...
				buttonAlign: "center",
				bodyStyle: "padding:10px 0 0px 15px",
				items: [{
					xtype: 'xDateTime',
					fieldLabel: '发送时间',
					layout: 'form',
				...
				items:[
				{
					...
					width: 500,
					validator: function(){
						var filePath = mainForm.items.items[8].getValue();
						if (filePath != '') {
							mainForm.items.items[9].el.dom.value = getFileMessage(filePath);
							document.getElementById("Js.Center.SendMMS.MMSsendUpdateDiy.ClientFileName").value = escape(filePath);
	                ...
	                id: "Js.Center.SendMMS.MMSsendUpdateDiy.ClientFileName"
	            },
	            {
					xtype: 'hidden',
    ...
    if (Js.Center.SendMMS.MMSsendUpdateDiy.MMSSendUpdateWin == null) {
		...
		Js.Center.SendMMS.MMSsendUpdateDiy.EditPanel = new Ext.FormPanel({
			...
			items: [{
				...
				style: 'border: 1px solid #C0C0C0;height:22;cursor:hand',
				name: 'vc2image',
				fieldLabel: getHelpMsg("帧图片", false, "此项非必选项，上传图片格式为：jpg|gif|jpeg|bmp|png，尺寸最大限制为" + mmsConfigInfo.imagewidth + " x " + mmsConfigInfo.imageheight + "（宽x高），大小不能超过"+mmsConfigInfo.imagesize/1024+"K；选中右侧“不需要”项，则删除当前帧已上传的图片！"),
				width: 200
			}, {
				xtype: 'FileUpload',
				...
				style: 'border: 1px solid #C0C0C0;height:22;cursor:hand',
				name: 'vc2music',
				fieldLabel: getHelpMsg("帧背景音乐", false, "此项非必选项，上传音乐格式为：“midi”或“amr”，大小不能超过"+mmsConfigInfo.musicsize/1024+"K；选中右侧“不需要”项，则删除当前帧已上传的音乐！"),
				width: 350
			}, {
				xtype: 'textarea',
